...
from tqdm import tqdm  # 在文件顶部添加导入语句
...
def train():
    ...
    # 打印每个 epoch 的 step 数量（可选）
    print(f"每个 epoch 的训练 step 数量: {len(dataloader['train'])}")
    print(f"每个 epoch 的验证 step 数量: {len(dataloader['val'])}")
    ...
    for epoch in range(EPOCH):
        ...
        # 训练阶段，添加进度条
        model.train()
        for idx, sample in enumerate(tqdm(dataloader['train'], desc=f"Epoch {epoch+1} 训练")):
        ...
        # 验证阶段，添加进度条
        model.eval()
        for idx, sample in enumerate(tqdm(dataloader['val'], desc=f"Epoch {epoch+1} 验证")):
            image = sample['image'].to(device=device)
            question = sample['question'].to(device=device)
